...
p_false_pos = 0.1 # 0 is perfect, towards 1 is noisy
p_false_neg = 0.1 # 0 is perfect, towards 1 is noisy
...
if fixed_seed:
    ...
    setup = {
            ...
            "fov": fov_dict["fov"],
            "fov_size": fov_dict["fov_size"]}
else:
    # save setup
    setup = {
            "notes": notes,
            "seed": 'different seeds used',
            "num_trials": num_trials,
            "size_x": size_x,
            "size_y": size_y,
            "num_agents": num_agents,
...
for i in range(num_trials): # loop through the number of trials
    ...
    if fixed_seed:
        trial_seed = seed  # every trial will use the same seed
    else:
        trial_seed = np.random.randint(0, 2**32)  # each trial will have a randomly generated seed
    rng_seed = rand.default_rng(trial_seed)
    ...
    search_distribution, peaks, centers = gaussian_filter(trial_seed, size_x, size_y, num_objects, num_peaks_range, peak_height_range, peak_width_range_x, peak_width_range_y, peak_rot_range)
    ...
    env = gym.make("irl_gym_multi_search/multi_GridWorldEnv-v0",
                   max_episode_steps=max_steps,
                   params=param,
                   start_pos=start,
                   start_dir=start_dir,
    ...
    while not done: # trial loop. this loop runs until the end of the single trial
        ...
        actions = rng_seed.choice(4, size=num_agents)
